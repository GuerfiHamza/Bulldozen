...
@section('page_header')
    <div class="container-fluid">
        ...
        @can('add',app($dataType->model_name))
        <a href="{{ route('od.export')}}" class="btn btn-primary btn-add-new">
           <i class="voyager-cloud-download"></i> <span> Export</span>
        </a>
        @endcan
        @can('add',app($dataType->model_name))
        <a href="{{ route('od.export')}}" class="btn btn-primary btn-add-new">
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            <td>
                                                ...
                                                @elseif($row->type == 'file' && !empty($data->{$row->field}) )
                                                    @include('voyager::multilingual.input-hidden-bread-browse')
                                                    @if(json_encode($data->{$row->field}))
                                                        @foreach(json_encode($data->{$row->field}) as $file)
                                                ...
                                                @elseif($row->type == 'multiple_images')
                                                    @php $images = json_encode($data->{$row->field}); @endphp
                                                    @if($images)

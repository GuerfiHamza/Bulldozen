            ...
            <div class="col-md-12">
                ...
                <div class="panel panel-bordered" style="padding-bottom:5px;">
                    ...
                    @foreach($dataType->readRows as $row)
                        @php $rowDetails = json_encode($row->details);
                         if($rowDetails === null){
                        ...
                        <div class="panel-body" style="padding-top:0;">
                            ...
                            @elseif($row->type == 'multiple_images')
                                @if(json_encode($dataTypeContent->{$row->field}))
                                    @foreach(json_encode($dataTypeContent->{$row->field}) as $file)
                                        <img class="img-responsive"
                            ...
                            @elseif($row->type == 'select_multiple')
                                @if(property_exists($rowDetails, 'relationship'))
                                    ...
                                    @foreach(json_encode($dataTypeContent->{$row->field}) as $item)
                            ...
                            @else
                                @include('voyager::multilingual.input-hidden-bread-read')
                                @if ($row->display_name == 'Total' || $row->display_name == 'Sous total' || $row->display_name == 'Tax' || $row->display_name == 'Réduction'|| $row->display_name == 'Billing Shipping')
                                    <p>{{ $dataTypeContent->{$row->field} / 100 }} DZD</p>
                                @else
                                    <p>{{ $dataTypeContent->{$row->field} }}</p>
                                @endif
                            @endif
